...
parser.add_argument(
    ...
    "--articles",
    type=str,
    help="Path to folder with all articles (.json.gz)",
    required=True,
)
parser.add_argument(
    "-c",
...
parser.add_argument(
    ...
    "--speakers",
    type=str,
    help="Path to the augmented speakers folder (.parquet.gz)",
    required=True,
)
parser.add_argument(
    "-r",
    "--results",
    type=str,
    help="Path to the results from the inference step (.csv)",
    required=True,
)
parser.add_argument(
...
parser.add_argument(
    ...
    nargs="+",
    default=["year", "month"],
    help="Column to partition the out with. A choice of year and month. Order matters. Default 'year month'",
)

args = parser.parse_args()

spark = SparkSession.builder.appName("processRes").getOrCreate()
qc = spark.read.json(args.context)
articles = spark.read.json(args.articles)
speakers = spark.read.parquet(args.speakers)
res = spark.read.csv(
    args.results,
    header=True,
...
quote_article_link = (
    ...
    .agg(
        ...
        get_website(F.sort_array(F.collect_list(F.struct("date", "website")))).alias(
            "urls"
        ),
    )
...
agg_proba.write.parquet(
    join(args.output, "quotebank-cache-proba"),
    mode="overwrite",
    compression="gzip",
)
agg_proba = spark.read.parquet(join(args.output, "quotebank-cache-proba"))
...
probas = (
    ...
    .agg(
        F.collect_list(F.struct(F.col("speaker"), F.col("proba"), F.col("qids"))).alias(
            "probas"
        )
    )
)
...
final.write.parquet(
    join(args.output, "quotebank-cache1"), mode="overwrite", compression="gzip"
)
final = spark.read.parquet(join(args.output, "quotebank-cache1"))
...
final.select(*SMALL_COLS).repartition(*args.partition).write.partitionBy(
    *args.partition
    ...
    join(args.output, "quotes-df"), mode="overwrite", compression="bzip2",
)
...
individual_probas = (
    res.filter("proba > 0")
    .orderBy("articleUID", "articleOffset", "rank")
...
article_df.repartition(*args.partition).write.partitionBy(*args.partition).parquet(
    join(args.output, "article-df"), mode="overwrite", compression="gzip",
)
