...
public class Utils {
	...
	public static SpeakerAlias findUniqueSuperstring(List<Token> needle,
			Iterable<SpeakerAlias> haystack, boolean caseSensitive) {
		SpeakerAlias bestMatch = null;
		for (SpeakerAlias candidate : haystack) {
			List<Token> candTok = Token.getTokens(candidate.getAlias());
			for (int i = 0; i < candTok.size() - needle.size() + 1; i++) {
				List<Token> subCandidate = candTok.subList(i, i + needle.size());
				boolean equals;
				if (caseSensitive) {
